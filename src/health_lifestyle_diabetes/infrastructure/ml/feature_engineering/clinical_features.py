...
class ClinicalFeatureEngineer:
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        ...
        df["hdl_to_ldl_ratio"] = df["hdl_cholesterol"] / df["ldl_cholesterol"].replace(0, np.nan)
        df["cholesterol_ratio"] = df["cholesterol_total"] / df["hdl_cholesterol"].replace(0, np.nan)
