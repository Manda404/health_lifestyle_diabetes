...
def plot_categorical_proportions(df: pd.DataFrame, column: str):
    ...
    logger_proportions.info(f"Visualisation de la variable catégorielle {column} terminée.\n")

    return fig


def plot_target_distribution_within_category(df: pd.DataFrame, cat_col: str, target_col: str):
    """
    ...
    logger_proportions_target.info(f"Analyse cat -> target avec facettes : {cat_col} → {target_col}")
    ...
    crosstab["proportion"] = (
        crosstab.groupby(cat_col)["count"]
...
def plot_target_distribution(df: pd.DataFrame, target_col: str):
    ...
    color_map = {cat: base_colors[i % len(base_colors)] for i, cat in enumerate(unique_categories)}
...
def plot_cumulative_distribution(df: pd.DataFrame, col: str) -> pd.DataFrame:
    ...
    summary = (
        pd.DataFrame({"category": counts.index, "count": counts.values, "percent": percent})
        .sort_values("count")
