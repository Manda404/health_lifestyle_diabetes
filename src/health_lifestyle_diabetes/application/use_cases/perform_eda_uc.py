...
class PerformEDAUseCase:
    ...
    def execute(self, df: DataFrame):
        ...
        if df is None or df.empty:
            self.logger.error("Dataset vide : impossible d'exécuter l'EDA.")
            raise ValueError("Le DataFrame fourni est vide.")

        self.logger.info("===== DÉBUT DU USE CASE : EDA =====")
        ...
        self.logger.info("Résumé dataset + détection types terminés.")

        # -------------------------------------------------------------
        # 2) Analyse de la cible
        # -------------------------------------------------------------
        self.logger.info("Analyse de la variable cible...")
        ...
        if self.risk_score_col in df.columns and self.diabetes_stage_col in df.columns:
            self.logger.info("Analyse du score de risque...")
            ...
            self.logger.warning(
                "Colonnes score/stage absentes : analyse du risque ignorée."
            )

        # -------------------------------------------------------------
        # 4) Analyse numérique : chaque variable numérique vs cible
        # -------------------------------------------------------------
        self.logger.info("Analyse des variables numériques...")
        for col in numeric_cols:
            ...
            try:
                ...
                self.logger.warning(f"Impossible d'analyser '{col}' : {e}")

        self.logger.info("===== FIN DU USE CASE : EDA =====")
